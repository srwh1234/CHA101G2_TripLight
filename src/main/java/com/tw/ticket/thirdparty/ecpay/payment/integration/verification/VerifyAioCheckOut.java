package com.tw.ticket.thirdparty.ecpay.payment.integration.verification;
...
import com.tw.ticket.thirdparty.ecpay.payment.integration.ecpayOperator.PaymentVerifyBase;
...
public class VerifyAioCheckOut extends PaymentVerifyBase {
	...
	public String getAPIUrl(final String mode) throws EcpayException {
		Element ele = (Element) doc.getElementsByTagName("AioCheckOut").item(0);
		String url = "";
		final NodeList nodeList = ele.getElementsByTagName("url");
	...
	public void verifyParams(final Object obj) {
		...
		final Element ele = (Element) doc.getElementsByTagName("AioCheckOut").item(0);
		final NodeList nodeList = ele.getElementsByTagName("param");
		for (int i = 0; i < nodeList.getLength(); i++) {
			final Element tmpEle = (Element) nodeList.item(i);
	...
	public void verifyInvoice(final InvoiceObj obj) {
		// 1. 比對特殊欄位值相依需求
		...
		if (obj.getCarruerType().isEmpty() || obj.getCarruerType().equals("1")) {
			...
			final Pattern r = Pattern.compile("[A-Za-z]{2}[0-9]{14}");
			final Matcher m = r.matcher(obj.getCarruerNum());
			if (!m.find())
			...
			final Pattern r = Pattern.compile("^\\/[A-Za-z0-9\\s+-]{7}$");
			final Matcher m = r.matcher(obj.getCarruerNum());
			if (!m.find())
		...
		if (obj.getInvoiceItemName().contains("|")) {
			final int itemCount = obj.getInvoiceItemName().split("|").length;
			int paramCount = 0;
			final Pattern r = Pattern.compile("(\\|\\||^\\||\\|$)");
			final Matcher invCount = r.matcher(obj.getInvoiceItemCount());
			final Matcher invWord = r.matcher(obj.getInvoiceItemWord());
			final Matcher invPrice = r.matcher(obj.getInvoiceItemPrice());
			final Matcher invType = r.matcher(obj.getInvoiceItemTaxType());
			if (invCount.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemCount("
							+ paramCount
							+ ") not match item count from InvoiceItemName("
							+ itemCount
							+ ")");
			}
			if (invWord.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemWord("
							+ paramCount
							+ ") not match item count from InvoiceItemName("
							+ itemCount
							+ ")");
			...
			if (invPrice.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemPrice("
							+ paramCount
							+ ") not match item count from InvoiceItemName("
							+ itemCount
							+ ")");
			...
			if (invType.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemTaxType("
							+ paramCount
							+ ") not match item count from InvoiceItemName("
							+ itemCount
							+ ")");
			...
			final String[] itemTax = obj.getInvoiceItemTaxType().split("|");
			for (final String tax : itemTax) {
