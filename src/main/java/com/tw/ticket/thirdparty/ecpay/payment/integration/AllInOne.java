...
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.ATMRequestObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutALL;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutATM;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutApplePay;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutBARCODE;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutCVS;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutDevide;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutOneTime;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutPeriod;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.AioCheckOutWebATM;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.CVSOrBARCODERequestObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.CreateServerOrderObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.DoActionObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.FundingReconDetailObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.InvoiceObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.QueryCreditCardPeriodInfoObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.QueryTradeInfoObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.QueryTradeObj;
import com.tw.ticket.thirdparty.ecpay.payment.integration.domain.TradeNoAioObj;
...
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyAioCheckOut;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyCreateServerOrder;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyDoAction;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyFundingReconDetail;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyQueryCreditTrade;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyQueryTrade;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyQueryTradeInfo;
import com.tw.ticket.thirdparty.ecpay.payment.integration.verification.VerifyTradeNoAio;
...
public class AllInOne extends AllInOneBase {
	 ...
	 */
	public AllInOne(final String log4jPropertiesPath) {
		super();
		if (log4jPropertiesPath != "" && log4jPropertiesPath != null) {
			...
			try {
				...
				final File conFile = new File(propertiesFile);
	 ...
	 * @return boolean
	 */
	public boolean compareCheckMacValue(final Hashtable<String, String> params) {
	 ...
	 * @return
	 */
	public String createServerOrder(final CreateServerOrderObj obj) {
		...
		try {
			...
			final VerifyCreateServerOrder verify = new VerifyCreateServerOrder();
			...
			final String httpValue = EcpayFunction.genHttpValue(obj, CheckMacValue);
	 ...
	 */
	public String fundingReconDetail(final FundingReconDetailObj fundingReconDetailObj) {
		fundingReconDetailObj.setMerchantID(MerchantID);
		log.info("fundingReconDetail params: " + fundingReconDetailObj.toString());
		String result = "";
		String CheckMacValue = "";
		try {
			final VerifyFundingReconDetail verify = new VerifyFundingReconDetail();
			...
			final String httpValue = EcpayFunction.genHttpValue(fundingReconDetailObj, CheckMacValue);
			log.info("fundingReconDetail post String: " + httpValue);
			result = EcpayFunction.httpPost(fundingReconDetailUrl, httpValue, "BIG5");
			final List<String> subRE = new ArrayList<>();
			...
			for (final String tmp : subRE) {
				result = result.replace(tmp, "\r\n" + tmp);
			}
			result = result.substring(2);
		} catch (final EcpayException e2) {
	 ...
	 * @return response string
	 */
	public String queryTrade(final QueryTradeObj queryTradeObj) {
		queryTradeObj.setMerchantID(MerchantID);
		log.info("queryTrade params: " + queryTradeObj.toString());
		String result = "";
		String CheckMacValue = "";
		try {
			final VerifyQueryTrade verify = new VerifyQueryTrade();
			...
			final String httpValue = EcpayFunction.genHttpValue(queryTradeObj, CheckMacValue);
	 ...
	 * @return response string
	 */
	public String tradeNoAio(final TradeNoAioObj tradeNoAioObj) {
		tradeNoAioObj.setMerchantID(MerchantID);
		log.info("tradeNoAio params: " + tradeNoAioObj.toString());
		String result = "";
		String CheckMacValue = "";
		try {
			final VerifyTradeNoAio verify = new VerifyTradeNoAio();
			...
			final String httpValue = EcpayFunction.genHttpValue(tradeNoAioObj, CheckMacValue);
			...
			if (tradeNoAioObj.getMediaFormated().equals("0")) {
				final Pattern pattern = Pattern.compile("\\d{4}\\-\\d{2}\\-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{16}");
	 ...
	 * @return response string
	 */
	public String doAction(final DoActionObj doActionObj) {
		...
		String CheckMacValue = "";
		try {
			final VerifyDoAction verify = new VerifyDoAction();
			...
			final String httpValue = EcpayFunction.genHttpValue(doActionObj, CheckMacValue);
	 ...
	 * @return response string
	 */
	public String queryTradeInfo(final QueryTradeInfoObj queryTradeInfoObj) {
		...
		String CheckMacValue = "";
		try {
			final VerifyQueryTradeInfo verify = new VerifyQueryTradeInfo();
			...
			final String httpValue = EcpayFunction.genHttpValue(queryTradeInfoObj, CheckMacValue);
	 ...
	 */
	public String queryCreditCardPeriodInfo(final QueryCreditCardPeriodInfoObj queryCreditCardPeriodInfoObj) {
		...
		String CheckMacValue = "";
		try {
			final VerifyQueryCreditTrade verify = new VerifyQueryCreditTrade();
			...
			final String httpValue = EcpayFunction.genHttpValue(queryCreditCardPeriodInfoObj, CheckMacValue);
	 ...
	 * @param String
	 */
	public String aioCheckOut(final Object obj, final InvoiceObj invoice) {
		...
		try {
			final VerifyAioCheckOut verify = new VerifyAioCheckOut();
	 ...
	 * @return obj
	 */
	public Object aioCheckOutFeedback(final HttpServletRequest req) {
		final List<String> parameterNames = new ArrayList<>(req.getParameterMap().keySet());
		if (parameterNames.contains("BankCode")) {
			final ATMRequestObj obj = new ATMRequestObj();
			...
			final CVSOrBARCODERequestObj obj = new CVSOrBARCODERequestObj();
			for (final String name : parameterNames) {
				Method method;
				try {
					method = obj.getClass().getMethod("set" + name, null);
	 ...
	 * @return string
	 */
	private String genCheckOutHtmlCode(final Object aio, final InvoiceObj invoice) {
		final StringBuilder builder = new StringBuilder();
		final Hashtable<String, String> fieldValue = EcpayFunction.objToHashtable(aio);
		Hashtable<String, String> invoiceField = new Hashtable<>();
		if (invoice != null) {
			invoiceField = EcpayFunction.objToHashtable(invoice);
			fieldValue.putAll(invoiceField);
		}
		final String CheckMacValue = EcpayFunction.genCheckMacValue(HashKey, HashIV, fieldValue);
		fieldValue.put("CheckMacValue", CheckMacValue);
		final Set<String> key = fieldValue.keySet();
		final String name[] = key.toArray(new String[key.size()]);
